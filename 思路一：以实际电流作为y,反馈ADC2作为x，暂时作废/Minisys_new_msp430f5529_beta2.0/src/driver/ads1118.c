...
float a0 = 0, a1 = 0, a2 = 0, CC_value_fact = 0, CV_value_fact = 0, current = 0; //电流值变量;
char c[12] = {};
char diaplay_i_flag = 0;
...
void cc_mode_adc2_feedback(float v)
{
    ...
    if (v <= 0.825)
        CC_value_fact = 0.0205 * pow(v, 2) + 0.8528 * v - 0.0066;
    else if (v <= 1.65)
        CC_value_fact = -0.01 * pow(v, 2) + 0.8805 * v - 0.0125;
    else if (v <= 2.475)
        CC_value_fact = -0.009 * pow(v, 2) + 0.8759 * v - 0.0087;
    else
        CC_value_fact = 0.004 * pow(v, 2) + 0.7899 * v + 0.1241;
    //差值判断程序块
    temp = CC_value_fact - CC_value;
    if (temp >= 1) //差值temp>=1  减量  1
    {
        CC_voltage -= 1;
    }
    else if (temp >= 0.5) //差值0.5<=temp<1   减量    0.2
    {
        CC_voltage -= 0.2;
    }
    else if (temp >= 0.1) //差值0.1<=temp<0.5  减量    0.15
    {
        CC_voltage -= 0.15;
    }
    else if (temp >= 0.02) //差值0.02<=temp<0.1   减量  0.002
    {
        CC_voltage -= 0.002;
    }
    else if (temp >= 0) //差值0<=temp<0.02    减量0.001
    {
        CC_voltage -= 0.001;
    }
    else if (temp >= -0.02) //差值-0.02<=temp<0   增量=0.001
    {
        CC_voltage += 0.001;
    }
    else if (temp >= -0.1) //差值-0.1<=temp<-0.02  增量=0.002
    {
        CC_voltage += 0.002;
    }
    else if (temp >= -0.5) //差值-0.5<=temp<-0.1  增量=0.15
    {
        CC_voltage += 0.15;
    }
    else if (temp >= -1) //差值-1<=temp<-0.5   增量=0.2
    {
        CC_voltage += 0.2;
    }
    else //差值temp<-2A   增量=1
...
void get_ads1118_sample_voltage(void)
{
    ...
    case 1:
        ...
        ssd1351_display_string(17, 100, v0, 1, White, Black);
        ssd1351_display_string(17, 100, c, 1, White, Black);
        break;
    case 2:
