...
float a0 = 0, a1 = 0, a2 = 0, CC_value_fact = 0, CV_value_fact = 0, dianyuan=0,CR_value_fact=0,CR_current_fact=0; //电流值变量;
...
{
    float temp = 0;
    CR_current_fact = -0.0067*pow(v,2) + 0.9576*v - 0.0009;
    //实际的电流显示与采样电压ADC2的关系，根据ADC2能够得到实际的电流，以便于下方与用户设定电流值做对比，看插值
    //差值判断程序块
    temp = CR_current_fact - CR_current;
    if((temp>=0.002)&&(temp<=0.003))
    {
        CR_voltage-=0.0108*pow(temp,2) + 0.9408*temp + 0.0024;
    }
    else if((temp>=-0.003)&&(temp<=-0.002))
    {
        CR_voltage+=0.0108*pow(temp,2) + 0.9408*temp + 0.0024;
...
void get_ads1118_sample_voltage(void)
{
    ...
    case 2:
        sprintf(v0, "%.3fV ", dianyuan); //A1通道的电压
        ssd1351_display_string(80, 80, v0, 0, White, Black);
        sprintf(v0, "%.3fA ", CR_current); //A1通道的电压
        ssd1351_display_string(14, 92, v0, 0, White, Black);
        CR_value_fact=(dianyuan*1.0)/CR_current;
        sprintf(v0, "%.2f ", CR_value_fact); //A1通道的电压
        ssd1351_display_string(71, 104, v0, 0, White, Black);
        break;
    case 3:
        a2 = average_voltage_caculate(6, 10);
