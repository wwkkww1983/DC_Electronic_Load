...
void mode_choose_interface()
{
    ...
    ssd1351_display_string(0, 20, "  CR Mode", 1, White, Black);
    ssd1351_display_string(0, 40, "  Developer Mode", 1, White, Black);
}
    ...
    if ((past == 1) && (now == 0))
    {
        ...
        case 1:
            if (jiemian == 1) //处于模式选择界面时，KEY1的功能
            {
                if (jiantou == 1)//箭头1按下KEY1
                {
                    ...
                    ssd1351_display_string(0, 40, ">>", 1, White, Black);
                }
                else if (jiantou == 2)//箭头3按下KEY1
                {
                    jiantou = 1;
        ...
        case 3:
            ...
            else
            {
                ...
                if(jiantou==1)
                {
                    CC_voltage=0.0108*pow(CC_value,2) + 0.9408*CC_value + 0.0024;
                    DAC8571_OUT(CC_voltage);
                }
                else if(jiantou==2)
                {
                    CR_current=(dianyuan*1.0)/CR_value;
                    CR_voltage=0.0108*pow(CR_current,2) + 0.9408*CR_current + 0.0024;
                    DAC8571_OUT(CR_voltage);
                }
                else
                {
                    DAC8571_OUT(CV_value);
                }
            }
            break;
        case 4:
